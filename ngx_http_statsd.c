...
typedef struct {
    ...
    ngx_resolver_connection_t      *resolver_connection;
    ngx_log_t                 *log;
} ngx_udp_endpoint_t;
...
ngx_http_statsd_handler(ngx_http_request_t *r)
{
	...
	for (c = 0; c < ulcf->stats->nelts; c++) {
		...
		if (b == 0 || s.len == 0 || n <= 0) {
			...
			ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->udp->log, 0, "statsd: no value to send");
         	continue;
		};
...
static ngx_int_t ngx_statsd_init_endpoint(ngx_conf_t *cf, ngx_udp_endpoint_t *endpoint) {
    ...
    endpoint->resolver_connection = rec;
...
ngx_statsd_updater_cleanup(void *data)
{
    ...
    if(e->resolver_connection) {
        if(e->resolver_connection->udp) {
            ngx_close_connection(e->resolver_connection->udp);
        }

        ngx_free(e->resolver_connection);
    }
}
